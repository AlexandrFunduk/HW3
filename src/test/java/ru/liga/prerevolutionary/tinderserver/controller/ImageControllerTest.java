...
class ImageControllerTest {
    ...
    void searchNext() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/users/image/search/next").header("chatId", "2"))
                .andExpect(status().isOk())
    ...
    void searchPrevious() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/users/image/search/previous").header("chatId", "2"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.chatId").value("8"))
                .andExpect(jsonPath("$.image").exists());
    }

    @Test
    void viewNext() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/users/image/view/next").header("chatId", "2"))
                .andExpect(status().isOk())
    ...
    void viewPrevious() throws Exception {
        this.mockMvc.perform(MockMvcRequestBuilders.get("/users/image/view/previous").header("chatId", "2"))
                .andExpect(status().isOk())
